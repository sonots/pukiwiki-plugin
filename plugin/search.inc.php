...
function plugin_search_search_form($s_word = '', $type = '', $bases = array())
{
	...
	if (!empty($bases)) {
		...
		foreach($bases as $base) {
			...
			$label_id = 'p_search_base_id_' . $_num;
...
EOD;
		...
		$base_msg .=<<<EOD
  <input type="radio" name="base" id="p_search_base_id_all" value="" />
  <label for="p_search_base_id_all">$_search_all</label>
EOD;
	...
	if (! PLUGIN_SEARCH_DISABLE_GET_ACCESS) {
	return <<<EOD
<form class="search" action="$script" method="get">
 <div>
  <input type="hidden" name="cmd" value="search" />
  <input class="word" type="text"  name="word" value="$s_word" size="20" />
  <input class="and" type="radio" name="type" id="p_search_AND" value="AND" $and_check />
  <label for="p_search_AND">$_btn_and</label>
  <input class="or" type="radio" name="type" id="p_search_OR" value="OR"  $or_check />
  <label for="p_search_OR">$_btn_or</label>
  &nbsp;<input class="submit" type="submit" value="$_btn_search" />
 </div>
$base_option
</form>
EOD;
	...
	return <<<EOD
<form class="search" action="$script?cmd=search" method="post">
 <div>
  <input class="word" type="text"  name="word" value="$s_word" size="20" />
  <input class="and" type="radio" name="type" id="p_search_AND" value="AND" $and_check />
  <label for="p_search_AND">$_btn_and</label>
